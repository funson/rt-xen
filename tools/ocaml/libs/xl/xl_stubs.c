...
/* RT-Xen */
static int sched_gedf_val(caml_gc *gc, libxl_sched_gedf *c_val, value v)
{
	CAMLparam1(v);
	c_val->period = Int_val(Field(v, 0));
	c_val->budget = Int_val(Field(v, 1));
	CAMLreturn(0);
}

static value Val_sched_gedf(libxl_sched_gedf *c_val)
{
	CAMLparam0();
	CAMLlocal1(v);
	...
	Store_field(v, 0, Val_int(c_val->period));
	Store_field(v, 1, Val_int(c_val->budget));
...
/* RT-Xen */
value stub_xl_sched_gedf_domain_get(value domid)
{
	CAMLparam1(domid);
	CAMLlocal1(scinfo);
	libxl_sched_gedf c_scinfo;
	int ret;
	INIT_STRUCT();

	INIT_CTX();
	ret = libxl_sched_gedf_domain_get(&ctx, Int_val(domid), &c_scinfo);
	if (ret != 0)
		failwith_xl("sched_gedf_domain_get", &lg);
	FREE_CTX();
	
	scinfo = Val_sched_gedf(&c_scinfo);
	CAMLreturn(scinfo);
}

value stub_xl_sched_gedf_domain_set(value domid, value scinfo)
{
	...
	sched_gedf_val(&gc, &c_scinfo, scinfo);

	INIT_CTX();
	ret = libxl_sched_gedf_domain_set(&ctx, Int_val(domid), &c_scinfo);
	if (ret != 0)
		failwith_xl("sched_gedf_domain_set", &lg);
	FREE_CTX();
