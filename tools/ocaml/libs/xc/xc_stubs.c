...
/* RT-Xen */
CAMLprim value stub_sched_gedf_domain_get(value xch, value domid)
{
	CAMLparam2(xch, domid);
	CAMLlocal1(sdom);
	struct xen_domctl_sched_gedf c_sdom;
	int ret;

	// caml_enter_blocking_section();
	ret = xc_sched_gedf_domain_get(_H(xch), _D(domid), &c_sdom);
	// caml_leave_blocking_section();
	if (ret != 0)
		failwith_xc(_H(xch));

	sdom = caml_alloc_tuple(4);
	Store_field(sdom, 0, Val_int(c_sdom.period));
	Store_field(sdom, 1, Val_int(c_sdom.budget));
	Store_field(sdom, 2, Val_int(c_sdom.vcpu));
	Store_field(sdom, 3, Val_int(c_sdom.extra));

	CAMLreturn(sdom);
}

CAMLprim value stub_sched_gedf_domain_set(value xch, value domid,
                                            value sdom)
{
	CAMLparam3(xch, domid, sdom);
	struct xen_domctl_sched_gedf c_sdom;
	...
	c_sdom.period = Int_val(Field(sdom, 0));
	c_sdom.budget = Int_val(Field(sdom, 1));
	c_sdom.vcpu = Int_val(Field(sdom, 2));
	c_sdom.extra = Int_val(Field(sdom, 3));
	// caml_enter_blocking_section();
	ret = xc_sched_gedf_domain_set(_H(xch), _D(domid), &c_sdom);
	// caml_leave_blocking_section();
	if (ret != 0)
		failwith_xc(_H(xch));
